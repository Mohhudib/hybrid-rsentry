...
> **Prerequisite (open §7-Q4):** ablation requires **per-layer enable/disable toggles** that don't otherwise perturb the agent. These likely do not all exist today (e.g. canary can be suppressed by not seeding/registering; rename/write-offset/entropy need config gates). And **entropy is currently an async severity *enrichment*, not an independent containing trigger (§0.4/§7-Q2)** — if it cannot independently cause containment, "disable entropy" measures a severity-grade change, not a detection-rate drop, and must be analyzed as such (or the metric for that layer changes to "severity downgrade rate").
...
2. **Layer attribution & entropy's status (§0.4, §3.2).** Confirm "first-firing layer wins." Critically: **is entropy currently able to *independently* trigger containment, or is it only an async severity enrichment?** If the latter, "entropy" is not a cleanly ablatable *containing* layer and its robustness metric becomes "severity-downgrade rate," not "detection-rate drop." This is the single biggest design fork.
