...
echo ""
echo " Run the backend (must source .env for DATABASE_URL):"
echo "   set -a && source .env && set +a && uvicorn backend.main:app --reload"
echo ""
echo " Run Celery workers (must source .env for DATABASE_URL + NVIDIA keys):"
echo "   set -a && source .env && set +a && PYTHONPATH=. celery -A backend.workers.tasks:celery_app worker --loglevel=info"
echo ""
echo " Run the agent (preserves env vars through sudo):"
echo "   set -a && source .env && set +a"
echo "   sudo -E venv/bin/python -m agent.monitor"
echo ""
