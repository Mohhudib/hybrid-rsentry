...
class TestEntropySeverity:
    @staticmethod
    def _spike(tmp_path):
        """Force a guaranteed entropy spike (zeros → random) and return the alert."""
        f = tmp_path/"t.bin"; e = EntropyEngine()
        f.write_bytes(b"\x00"*2048); e.observe(str(f))
        f.write_bytes(os.urandom(2048))
        r = e.observe(str(f))
        assert r is not None, "zeros→random rewrite must produce an alert"
        return r
    def test_alert_has_severity_field(self, tmp_path):
        r = self._spike(tmp_path)
        assert r["severity"] in ("MEDIUM","HIGH","LOW","CRITICAL")
